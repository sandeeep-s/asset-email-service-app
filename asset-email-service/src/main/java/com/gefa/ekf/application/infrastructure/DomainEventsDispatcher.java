...
import com.gefa.ekf.boundary.outbound.async.fit.EmailSentEventHandler;
...
public class DomainEventsDispatcher {
	...
	public DomainEventsDispatcher(@Any Instance<DomainEventHandler> handlerList) {
		...
		while (handlerIterator.hasNext()) {
			...
			System.out.println("EmailSentEventHandler.class="+EmailSentEventHandler.class);
			if (handler instanceof EmailSentEventHandler) {
				addHandler(EmailSentEvent.class, handler);
			}
		}
	}
