...
public class JMSFacade {
	...
	public void sendEmail(EmailEvent emailEvent) {
		try {
			sendMessage(emailEvent, "emailq.send", DeliveryMode.PERSISTENT, false);
		} catch (JMSException e) {
			e.printStackTrace();
		}
	}

	public void semdEmailSync(EmailEvent emailEvent) {
		long objectNumber = 0;
		try {
			TextMessage txtmsg = sendMessage(emailEvent, "emailq.send.sync", DeliveryMode.PERSISTENT, true);
